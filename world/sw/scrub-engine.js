       ...
       sections: [
         { id, label, still, stillMobile, clip, clipMobile, accent,
           scroll: 1.6,   // optional per-section override of diveScroll — more scroll
                          // distance = a slower, longer dwell in this scene. Dwell
                          // depth derives from this automatically: playback rate at
                          // every seam is equalized across the chain, so longer legs
                          // settle mid-scene (where the copy peaks) and every seam
                          // hands off at the same velocity — no per-seam speed step.
...
function mountScrollWorld(container, config) {
  ...
  // Velocity-locked chain: each segment's easing edge slope ∝ its scroll width, so
  // scrub rate (video-time per scroll-px) is IDENTICAL on both sides of every seam.
  // Longer legs then dwell deeper mid-scene for free. slope < 2 keeps the easing
  // monotone; the clamp only engages if one leg is >1.9× the shortest.
  const WMIN = SEGMENTS.reduce((m, s) => Math.min(m, s.w), Infinity);
  SEGMENTS.forEach(s => { s.slope = Math.min(s.w / WMIN, 1.9); });
  ...
  // segment scenes. Stacked in chain order (later segments on top) so each
  // incoming scene dissolves IN over the previous one during the seam window —
  // a real crossfade, not a z-swap cut at the boundary.
  SEGMENTS.forEach((s, i) => {
  ...
  // Per-segment dwell with velocity-matched seams: f(0)=0, f(1)=1, f'(0)=f'(1)=s,
  // mid-scene rate 2−s. With s = w/wMin the seam-edge scrub velocity is the same
  // constant chain-wide (no speed step at any seam) while wider legs settle
  // mid-scene, exactly where the copy peaks. s=1 → linear.
  const dwellEase = (x, s) => s * x + (2 - 2 * s) * (x / 2 - Math.sin(2 * Math.PI * x) / (4 * Math.PI));
  let vh = window.innerHeight, stageX = 0, totalW = 0, activeIndex = -1;
  let laidOutW = window.innerWidth;   // width the current layout was computed at (see onResize)
  // Smoothed scroll driver: every visual (crossfades, copy drift, scrub targets)
  // renders from `yS`, an exponentially-smoothed follower of the real scroll
  // position, updated in the rAF loop. Wheel scrolling arrives in ~100px steps;
  // binding visuals to raw scrollY makes dissolves and parallax jump in those
  // quanta. On touch the native momentum is already continuous (and coalesced
  // seeks add enough latency), so phones render from the raw position; ditto
  // under prefers-reduced-motion.
  const SCROLL_SPEED = 9;  // 1/s — higher = tighter follow, lower = floatier
  let yS = window.scrollY || window.pageYOffset, renderedY = NaN;
  ...
  // Force a re-render on the next frame (layout changed, a clip became ready…).
  function invalidate() { renderedY = NaN; }
  ...
  function render(y, rawY) {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      // fades in while ALREADY scrubbing, over the outgoing scene holding its
      // frame-locked last frame — the film never freezes through the band and never
      // lurches to speed at the seam. Outgoing releases only once fully covered.
      const tin = i === 0 ? 1 : smooth((y - s.start) / fade);
      const tout = i === NSEG - 1 ? 1 : smooth(1 - (y - s.end - fade) / fade);
  ...
  function raf(now) {
    ...
    // Clamped so a background-tab wakeup doesn't teleport mid-dissolve.
    const dt = clamp((now - lastT) / 1000, 0.001, 0.05); lastT = now;
    const rawY = window.scrollY || window.pageYOffset;
    if (reduce || isMobile()) yS = rawY;
    else {
      yS += (rawY - yS) * (1 - Math.exp(-SCROLL_SPEED * dt));
  ...
  // driver needs per-frame updates anyway, and the read is cheap.)
